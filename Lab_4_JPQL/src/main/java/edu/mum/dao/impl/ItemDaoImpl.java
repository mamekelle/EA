...
public class ItemDaoImpl extends GenericDaoImpl<Item> implements ItemDao {
	...
	public List<Item> findByCategoryName(String categoryName) {
		...
		Query query = entityManager.createNamedQuery("from Item");
	...
	public List<Item> findBySellerOrBuyer(Integer initialPrice, User buyer, User seller) {
		...
		// TODO Seller Test
		if (seller != null)
			sellerPrice = "(i.initialPrice > :price and i.seller = :seller )";
		// TODO Buyer test
		if (buyer != null)
		...
		Query query = entityManager
				.createQuery("select distinct i from Item i, User u where " + sellerPrice + or + buyerPrice);

		if(seller!=null) {
			query.setParameter("seller", seller).setParameter("price", price);
		}if(buyer!=null) {
